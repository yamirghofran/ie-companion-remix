...
import { getAllProfessors } from "~/util/db";
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const hasMore = professors.length === limit;

  return json({ professors, page, hasMore });
}

export default function Professors() {
  const { professors, page, hasMore } = useLoaderData<typeof loader>();

  return (
    <div className='flex flex-col items-center'>
      <h1 className='text-3xl font-semibold pt-12'>Professors</h1>
          ...
          >
            {
              professors.map((professor) => {
                return (
                  <li className="md:w-10/12 lg:w-[720px] w-full  border-b border-b-slate-300">
                  <Link
                    to={`/professors/${professor.id}`} key={professor.id}
                  ...
                  >
                    <div className="order-2 flex flex-col items-start justify-end">
                      <Courses courses={professor.courses.map(c => c.course)} />
                    </div>
...
const Courses = ({ courses }: { courses: Course[] }) => {
    return (
      <>
        {courses.map((course) => (
          <p key={course.id} className='text-sm text-slate-500'>{course.name}</p>
        ))}
      </>
    );
  };